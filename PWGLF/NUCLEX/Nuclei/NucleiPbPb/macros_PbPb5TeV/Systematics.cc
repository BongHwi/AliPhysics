...
void Systematics() {
  ...
  vector<TH1F*> corr_syst(n_centralities,nullptr);
  vector<TH1F*> uncorr_syst(n_centralities,nullptr);
  ...
  for (int iC = 0; iC < n_centralities; ++iC) {
    ...
    for (int iS = 0; iS < 2; ++iS) {
      ...
      // references[iC] = (TH1F*) references_tmp->Rebin(kNPtBins,Form("TOFspectra_%d",iC),pt_bin_limits);
      references[iC] = (TH1F*) references_tmp->Clone(Form("TOFspectra_%d",iC));
      ...
      // countsyst[iC] = (TH1F*)countsyst_tmp->Rebin(kNPtBins,Form("countsyst_%d",iC),pt_bin_limits);
      countsyst[iC] = (TH1F*) countsyst_tmp->Clone(Form("countsyst_%d",iC));
      ...
      // shiftsyst[iC] = (TH1F*)shiftsyst_tmp->Rebin(kNPtBins,Form("shiftsyst_%d",iC),pt_bin_limits);
      shiftsyst[iC] = (TH1F*) shiftsyst_tmp->Clone(Form("shiftsyst_%d",iC));
      ...
      matsyst[iC] = (TH1F*)matsyst_tmp->Rebin(kNPtBins,Form("hMatSyst_%d",iC),pt_bin_limits);
      ...
      */
      ...
      corr_syst[iC] = (TH1F*)countsyst[iC]->Clone(("corr_syst" + to_string(iC)).data());
      corr_syst[iC]->Reset();

      uncorr_syst[iC] = (TH1F*)countsyst[iC]->Clone(("uncorr_syst" + to_string(iC)).data());
      uncorr_syst[iC]->Reset();
      ...
      for (auto& syst : kCutNames) {
        ...
        std::cout << "test: " << basepath << std::endl;
        ...
        for (size_t iV = 0; iV < syst.second.size(); ++iV) {
          std::cout << "Load: " << Form(basepath.data(),iV) << std::endl;
          ...
          // variations[iV] = (TH1F*)variations_tmp->Rebin(kNPtBins,Form("variation_%zu",iV),pt_bin_limits);
          variations[iV] = (TH1F*) variations_tmp->Clone(Form("variation_%d",iC));
      ...
      for (int iB = 1; iB <= kNPtBins; ++iB) {
        ...
        // }
          ...
          corr_syst[iC]->SetBinContent(iB,corr);
          ...
          uncorr_syst[iC]->SetBinContent(iB,uncorr);
        }
      }

      TCanvas summary("summary","Summary");
      summary.DrawFrame(0.7,0.,kPtBins[kNPtBins-1],0.3,";#it{p}_{T} (GeV/#it{c}); Systematics uncertainties");
      ...
      corr_syst[iC]->Write("syst_corr");
      uncorr_syst[iC]->Write("syst_uncorr");
      summary.Write();
